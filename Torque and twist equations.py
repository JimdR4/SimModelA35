...
def Twist_x(G,J,R1y,z_hat,R2y,R3y,P,R_I,x):
    ...
    if x<x1:
        return (1/(G*J))*doubleintegral + C5
    
    elif (x2-0.5*xa)>x>=x1:
        return (1/(G*J))*(-R1y*(x-x1)*(-0.5* h_a-z_hat) + doubleintegral) + C5 
    
    elif x2>x>=(x2-0.5*xa):
        return (1/(G*J))*(-R1y*(x-x1)*(-0.5* h_a-z_hat) - R_I*np.sin(theta)*(x-x2+xa/2)*(-z_hat) + R_I*np.cos(theta)*(x-x2+xa/2)*0.5*h_a + doubleintegral) + C5
    
    elif (x2+0.5*xa)>x>=x2:
        return (1/(G*J))*(-R1y*(x-x1)*(-0.5* h_a-z_hat) - R_I*np.sin(theta)*(x-x2+xa/2)*(-z_hat) + R_I*np.cos(theta)*(x-x2+xa/2)*0.5*h_a - R2y*(x-x2)*(-0.5*h_a-z_hat) + doubleintegral)  + C5
        ...
        return (1/(G*J))*(-R1y*(x-x1)*(-0.5* h_a-z_hat) - R_I*np.sin(theta)*(x-x2+xa/2)*(-z_hat) + R_I*np.cos(theta)*(x-x2+xa/2)*0.5*h_a - R2y*(x-x2)*(-0.5*h_a-z_hat) \
                + P*np.sin(theta)*(x-x2-xa/2)*(-z_hat) - P*np.cos(theta)*(x-x2-xa/2)*(0.5*(h_a)) + doubleintegral) + C5
    
    elif x >= x3 :
        return (1/(G*J))*(-R1y*(x-x1)*(-0.5* h_a-z_hat) - R_I*np.sin(theta)*(x-x2+xa/2)*(-z_hat) + R_I*np.cos(theta)*(x-x2+xa/2)*0.5*h_a - R2y*(x-x2)*(-0.5*h_a-z_hat) \
                + P*np.sin(theta)*(x-x2-xa/2)*(-z_hat) - P*np.cos(theta)*(x-x2-xa/2)*(0.5*(h_a)) -R3y(x-x3)*(-0.5*h_a-z_hat) + doubleintegral) + C5
